...
export async function POST(req: Request) {
  try {
    const { examId, examInfo, examResults } = await req.json()
...
- Total Questions: ${examInfo.questionsData.length}
- Total Marks: ${examInfo.totalMarks}
- Duration: ${examInfo.duration} minutes
- Passing Score: ${examInfo.security.passingScore}%

STUDENT PERFORMANCE:
- Score: ${examResults.score}/${examInfo.totalMarks}
- Percentage: ${examResults.percentage}%
- Time Taken: ${Math.round(examResults.timeTaken / 60000)} minutes
...
- Student Answer: ${answer?.selectedAnswer || "Not answered"}
...
`
    ...
    try {
      ...
      analyticsData.timeAnalysis.timeDistribution = examInfo.questionsData.map((q: any, index: number) => {
        const answer = examResults.answers.find((a: any) => a.questionId === q.id)
        return {
      ...
      const fallbackAnalytics = generateFallbackAnalytics(examInfo, examResults)
