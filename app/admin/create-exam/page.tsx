...
export default function CreateExam() {
  ...
  const saveExam = async (status: "published" | "draft" = "published") => {
    ...
    if (!adminUser || !isAdmin) {
      setErrors(["Please log in as admin to create an exam"])
      return
    }
    ...
    try {
      ...
      // Use the admin API endpoint instead of direct database service
      const examPayload = {
      ...
      console.log('Sending exam data to admin API:', JSON.stringify(examPayload, null, 2))

      const response = await fetch('/api/debug-admin-create', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !result.success) {
        ...
        throw new Error(result.error || result.details || 'Failed to create exam')
      }

      console.log('Exam created successfully:', result)

      // Show success message and redirect
      alert(`Exam "${result.exam.title}" created successfully with ${result.questionsCreated} questions!`)
      router.push("/admin/exams")
    } catch (error) {
      console.error("Error saving exam:", error)
      setErrors([error instanceof Error ? error.message : "Failed to save exam. Please try again."])
    } finally {
      setIsSaving(false)
    }
  }
